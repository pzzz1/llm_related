...
奖励模型：使用奖励模型计算当前动作的即时奖励，也就是下面的 $r_ {t} $，不参与参数更新(其实在大模型背景里，奖励模型只能计算整个句子的奖励，本质上也就是最后一个token对应的action的奖励是准的，之前的action的奖励通过最后一个的奖励减去KL散度得到，意义有限，这也是PPO不适合大模型场景而引出GRPO的关键）  
