...
# 一些其他的基本概念
# 策略： RLHF中，策略表示的就是我们需要优化的担心，从策略进行采样的过程就是大模型生成句子的过程
# 单步奖励： 根据当前的状态，动作和下一个状态，通过奖励模型计算得到的即时奖励，评估当前动作的好坏
# 累计奖励： 一条完整的轨迹的单步奖励之和
# 折扣奖励： 平衡即时奖励和长期奖励之间的关系，使智能体决策时不仅考虑到当前的奖励，还要考虑到未来的潜在奖励
# 轨迹： 轨迹由一系列的状态和动作组成，一条完整的轨迹就是一次完整的采样，相当于大模型生成一条完整的句子
